...
There are certain operations that JavaScript will do at runtime when it needs to coerce data types. You can't trigger these operations directly as they are handled by the language, however the ECMAScript specification ([more specifically section 9](https://www.ecma-international.org/ecma-262/5.1/#sec-9)) covers some abstract operations that, while not explicitly part of the language, are defined to aid the specification of the semantics of the language.

We'll cover the most important ones `ToString`, `ToNumber` and `ToBoolean`. These three are the most important because all coercion in JavaScript will end up as one of these three primitive types. (You can't coerce a type into an `Object` or `Function` etc).

### ToString

Whenever any non-string value is coerced into a string, the underlying conversion is defined by the `ToString` abstraction operation. Here is a table representing how common primitive values will be coerced to a `string`.
...
Note that for _really really_ big or _really really_ small numbers, a coercion to `string` will result in the exponent form of the number.

`Objects`, unless you specify your own, will use the default `toString()` method on the `Object prototype` and will return the internal _[[Class]]_. For example "[object Object]"

`Arrays` have a default `toString()` method that will coerce each individual element in the array separately, and then join each element with ",". For example:
...
You may have come across the terms "falsy values" and "truthy values". When we refer to these terms, what we actually mean is "when coerced to a Boolean value, will the result be `true` or `false`".

There is a very small list of values that will coerce to `false`, and so it is much easier to just remember the list of "falsy" values. By definition, every other value will coerce to `true`. The "falsy" values are:
...
If the value is not in the table above, then it will coerce to `true`. Consider the following variables:
...
What do you think the results will be if we were to coerce the above variables into a `boolean`? That is, are they "truthy" or "falsy". They are all `strings`, and because none of them are the empty string value (""), they are all truthy!

One last point about `Boolean` coercion that Kyle Simpson covers in YDKJS is that JavaScript has actual keywords for `true` and `false`, and they behave as you'd expect for `boolean` values. It's a common misconception that the values `1` and `0` are identical to `true` and `false` respectively. While that may be true in other languages, in JavaScript, the numbers are numbers, and the booleans are booleans. You can coerce `1` to `true` (and vice versa), or `0` to `false` (and vice versa). But they are not the same.
