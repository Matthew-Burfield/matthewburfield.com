...
In all the above cases, we say they are type casting, because each data type is converted to another type _explicitly_. We say _explicit_ because the `String(..)` function and the `Number(..)` function make it super obvious that the data types are changing to a `number` and `string` respectively.

By contrast, **Corecion**, is changing a variable from one data type to another _implicitly_. Here we say _implicit_ to mean that it is not super obvious that the data type has changed, but we are instead using the set of rules imposed by the programming langauge, in how different data types are represented in different situations.

We're going to deep dive into what I mean by "the set of rules imposed by the programming language" later in the article, but for now, let's look at a couple of common examples of coercion in JavaScript. That is, _implicit_ conversion of data types.
...
You may have used something similar to the above in your code. It is a common pattern to evaluate if a string exists and has a value, but what's actually happening here? An `if block` cannot actually evaluate a string, it can only evaluation a boolean or an expression that returns a boolean... what's happening is that the language is coercing the `string` to a `boolean`.
...
But the language already knows that this is what we intend to do because of "the set of rules imposed by the programming language", therefore it _implicitly_ changes the data type for us. **That's coercion**.
