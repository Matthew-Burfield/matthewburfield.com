...
Using Amplify I've creating a graphQL API and I've need to query different things in a few different components - a perfect opportunity to create a hook to abstract that code!
...
const response = (await API.graphql(graphqlOperation(getData))) as { data: GetDataQuery };
```
...
As you'll soon see, React hooks look really familiar, they're pretty much just normal components, but instead of rendering something, they just return values that you are storing in state.
