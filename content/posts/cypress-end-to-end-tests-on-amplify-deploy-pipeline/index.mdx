...
+    "cy:run": "cypress run --record --key 428b202f-20f8-46ca-a99f-5c751d4869e0 --config baseUrl=http://localhost:5000",
...
2. **"cy:run"**: Using the "cypress run" command will run our e2e tests in headless mode. Which means that no browser window opens up, and the results are outputted to the console, perfect for when we are testing from our CI/CD server.

We pass to cypress the "record" flag to tell it to record the tests and send to the cypress dashboard;

The "key" parameter is required when cypress is recording to be able to send the output to the Cypress dashboard.

And finally, the "config" parameter tells Cypress to use the baseUrl on port 5000, instead of 3000. This is because when running our tests in CI, we are testing against the built project (using serve), instead of the development server.

3. **"ci:build-and-serve"**: This will just fire off two commands in sequence, first a build, and then "serve", which we just defined above.

4. **"ci:e2e"**: This is where it all comes together. First we kill any processes running on port 5000, just in case. Then we use a library called [start-server-and-test](https://www.npmjs.com/package/start-server-and-test) which will run the start server command (serve), and then wait for a response from the server before starting the tests. Exactly what we need.
...
```bash
git add .
git commit -m "Added CI build and test scripts"
git push origin master
```

Now take a sip of coffee and watch on the Amplify console as it builds, tests and deploys your project for you! ☕️✌
